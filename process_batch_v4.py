...
input_file = '/Users/shauryawadhawan/compace/raw_batch_4.csv'
...
def parse_date(date_str):
    ...
    # Check for "Month Day, Year" e.g. "January 7, 2026"
...
def detect_track(row):
    ...
    if any(x in text for x in ["coding", "computer science", "informatics", "algorithm", "cyber", "hackathon", "robotics"]):
        # Robotics is often Science but user might prefer Coding? 
        # Actually usually Robotics is Science/Engineering. 
        # But Coding has "algo", "cyber". 
        if "robotics" in text and "coding" not in text and "algorithm" not in text:
             return "Science" # Robotics defaults to Science unless explicit coding focus
        if "robotics" in text:
             return "Science" # Let's stick Robotics in Science to be safe (Engineering)
             
        return "Coding"

    # Priority 2: Math phrases (Check specific keywords)
    ...
    # Fallback based on "Olympiad"
    if "olympiad" in text:
        # Check if it has any science-y words not caught?
        # If bare Olympiad, maybe Math?
        return "Math"
...
def process_csv():
    ...
    for row in rows:
        # 1. Correct Track
        original_track = row.get('track', '')
        # User input has 'Econ', so we override if it's Econ or Olympiad (or empty)
        if original_track in ['Econ', 'Olympiad', '']:
    ...
    # Write to output
    # Use QUOTE_ALL to be safe with descriptions
